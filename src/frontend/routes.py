...
for root, dirs, files in os.walk("frontend/templates"):
  for file in files:
    ...
    with open(filepath, "r") as f:
      templates[
        filepath.removeprefix("frontend/templates").removeprefix("/")
      ] = f.read()
...
for root, dirs, files in os.walk("frontend/supporting"):
  for file in files:
    ...
    with open(filepath, "r") as f:
      sup_templates[
        filepath.removeprefix("frontend/supporting").removeprefix("/")
      ] = f.read()

for name, contents in templates.items():

  async def serve(
    request: web.Request, name=name, contents=contents
  ) -> web.Response:
