...
async def get_database_get(request: Request) -> Response:
  ...
  backup_count_record = await request.conn.fetchrow(
...
async def get_database_backups_list(request: Request) -> Response:
  ...
  backup_records = await request.conn.fetch(
    "SELECT * FROM Backups ORDER BY date DESC LIMIT $1 OFFSET $2;",
    limit,
    offset,
  )
  backup_count_record = await request.conn.fetchrow(
  ...
  backups_out = [
    {
      ...
      "date": datetime.datetime.fromtimestamp(record["date"]).strftime(
        "%Y/%m/%d-%H:%M:%S"
      ),
    }
